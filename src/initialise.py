    ...
    for obj in obj_list:
        if obj['name'] == name:
            ...
            if "noise" in obj['constraints']:
                ...
                noise[-1] = 0
            break
    # Build the list of arguments for loadURDF
    load_kwargs = {}
    # Use self-collision for the controlled robot/gripper; the original gripper mesh set
    # exceeds PyBullet's DAE visual-shape limit, so the URDF should still be loaded with
    # a simplified visual representation (see sisbot.urdf / robotiq_c2.urdf).
    if name == "ur5" or "robotiq_85" in urdf or "robotiq_c2" in urdf:
        load_kwargs['flags'] = p.URDF_USE_SELF_COLLISION | p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
    # For other objects, we keep default behavior
    if orientation == []:
        object_id = p.loadURDF(urdf, list(position+noise), **load_kwargs)
    else:
        object_id = p.loadURDF(urdf, list(position+noise), orientation, **load_kwargs)
    return (object_id, 
