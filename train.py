...
def load_dataset():
	...
	dataset_dir = "dataset/" + domain + "/"
	if not path.isdir(dataset_dir) or not any(path.endswith('.datapoint') for root, _, files in os.walk(dataset_dir) for path in [os.path.join(root, file) for file in files]):
		raise FileNotFoundError("Training dataset is missing or empty. Run generate_synthetic_dataset.py first.")
