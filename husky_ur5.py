...
CAMERA_RENDERER = p.ER_TINY_RENDERER if os.environ.get("TOOLNET_RENDERER", "hardware").lower() == "tiny" else p.ER_BULLET_HARDWARE_OPENGL
...
def start(input_args):
    ...
    # Connect to Bullet using GUI mode so the external simulation window stays visible.
    # Keep software rendering enabled to avoid Intel GPU hangs with PyBullet.
    ...
    # Camera capture is performed by the simulator thread because PyBullet's client
    # is not safe to access concurrently from a Python camera thread.
    start_camera_thread()
...
def executeHelper(actions, goal_file=None, queue_for_execute_to_stop = None, saveImg = True):
    ...
    if True:
        ...
        while(True):
            ...
            simulation_stats["steps"] += 1
            simulation_stats["step_seconds"] += step_seconds
            capture_camera_frame_if_due()
            time.sleep(max(0.0, (1.0 / SIMULATION_HZ) - step_seconds))
            ...
            if(actions[action_index][0] == "move"):
                ...
                if not waiting and not done:
                    ...
                    if actions[action_index][2] == 'ur5' and not "Movable" in properties[actions[action_index][1]]:
                        raise Exception("Object '" + actions[action_index][1] + "' is not grabbable")
                ...
                if horizontal_cons: horizontal_list.append(oid)
                if gnd: ground_list.append(oid)
...
def capture_camera_frame():
    global last_camera_capture
    capture_started = time.perf_counter()
    pos, orn = p.getBasePositionAndOrientation(husky)
    import math
    _, _, yaw = p.getEulerFromQuaternion(orn)
    behind_yaw = yaw + math.pi
    camera_pos = [pos[0] + 2.5 * math.cos(behind_yaw),
                  pos[1] + 2.5 * math.sin(behind_yaw), pos[2] + 1.2]
    target_pos = [pos[0], pos[1], pos[2] + 0.3]
    view_matrix = p.computeViewMatrix(camera_pos, target_pos, [0, 0, 1])
    proj_matrix = p.computeProjectionMatrixFOV(60, CAMERA_WIDTH / CAMERA_HEIGHT, 0.1, 10.0)
    w_img, h_img, rgb_img, _, _ = p.getCameraImage(
        CAMERA_WIDTH, CAMERA_HEIGHT, viewMatrix=view_matrix,
        projectionMatrix=proj_matrix, renderer=CAMERA_RENDERER,
        shadow=1, flags=p.ER_NO_SEGMENTATION_MASK)
    rgb_img = np.reshape(rgb_img, (h_img, w_img, 4))[:, :, :3]
    jpeg_data = io.BytesIO()
    Image.fromarray(rgb_img.astype(np.uint8)).save(jpeg_data, format='JPEG', quality=CAMERA_JPEG_QUALITY)
    ...
    try:
        ...
        try:
            ...
            pass
    simulation_stats["camera_frames"] += 1
    simulation_stats["camera_seconds"] += time.perf_counter() - capture_started
    last_camera_capture = time.monotonic()

def capture_camera_frame_if_due():
    if time.monotonic() - last_camera_capture >= 1.0 / CAMERA_HZ:
        try:
            capture_camera_frame()
        except Exception as error:
            print("Camera capture error:", error)

def idle_simulation_step():
    step_started = time.perf_counter()
    p.stepSimulation()
    step_seconds = time.perf_counter() - step_started
    simulation_stats["steps"] += 1
...
def start_camera_thread():
    ...
    if not camera_thread_started:
        ...
        print("Synchronous camera capture enabled")
